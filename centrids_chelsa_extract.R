...
rmse(pivot_df_1901_2016$Temperatura, pivot_df_1979_2019$Temperatura)
...
for (region in regiones) {
  ...
  rmse <- rmse(filtro_df_1901_2016$Temperatura, filtro_df_1979_2019$Temperatura)
  ...
  resultados[2,9] = correlacion
  resultados[2,10] = rmse
  
  resultados_fin <- rbind(resultados_fin, resultados)
}

resultados_fin <- resultados_fin[-1,]
write.csv2(resultados_fin, "C:/A_TRABAJO/ELIZA/CHELSA_transectos/tmax_comparation_regions.csv",row.names = FALSE, fileEncoding = "ISO-8859-1")
...
for (aa in transects) {
  ...
  # Calcular RMSE usando la función rmse()
  rmse <- rmse(filtro_df_1901_2016$Temperatura, filtro_df_1979_2019$Temperatura)
...
resultados_fin <- resultados_fin[-1,]
write.csv2(resultados_fin, "C:/A_TRABAJO/ELIZA/CHELSA_transectos/tmax_comparation_transects.csv",row.names = FALSE, fileEncoding = "ISO-8859-1")
...
resultados_fin <- resultados_fin[-1,]
write.csv2(resultados_fin, "C:/A_TRABAJO/ELIZA/CHELSA_transectos/tmin_comparation_regions.csv",row.names = FALSE, fileEncoding = "ISO-8859-1")
...
resultados_fin <- resultados_fin[-1,]
write.csv2(resultados_fin, "C:/A_TRABAJO/ELIZA/CHELSA_transectos/tmin_comparation_transects.csv", row.names = FALSE, fileEncoding = "ISO-8859-1")
...
## Procesar para 1901_2016 ----
# Lista de archivos raster
raster_files_1901_2016 <- list.files("D:/A_DATA/CHELSA/MONTHLY_1901_2016/PCP/", pattern = ".tif", full.names = TRUE)
...
for (raster_path in raster_files_1901_2016) {
  ...
  # Crear nombre de columna con formato MM_YYYY
  raster_name <- gsub("CHELSAcruts_prec_|_V.1.0|\\.tif", "", basename(raster_path))
  split_name <- unlist(strsplit(raster_name, "_"))
  mes <- sprintf("%02d", as.numeric(split_name[1]))  # Asegurar formato de dos dígitos
  anio <- split_name[2]
  transect_centr_df[[paste0(mes, "_", anio)]] <- raster_values
...
## Procesar para 1979_2019 ----
# Lista de archivos raster
raster_files_1979_2019 <- list.files("D:/A_DATA/CHELSA/MONTHLY_1979_2019/PCP/", pattern = ".tif", full.names = TRUE)
...
for (raster_path in raster_files_1979_2019) {
  ...
  # Crear nombre de columna con formato MM_YYYY
  raster_name <- gsub("pr_|\\.tif", "", basename(raster_path))
  split_name <- unlist(strsplit(raster_name, "_"))
  anio <- split_name[2]
  mes <- sprintf("%02d", as.numeric(split_name[1]))  # Asegurar formato de dos dígitos
  transect_centr_df[[paste0(mes, "_", anio)]] <- raster_values
...
# write.csv2(transect_centr_df_1901_2016,"C:/A_TRABAJO/ELIZA/CHELSA_transectos/pcp_transect_centr_df_1901_2016.csv",  
#            row.names = FALSE, fileEncoding = "ISO-8859-1")
# write.csv2(transect_centr_df_1979_2019,"C:/A_TRABAJO/ELIZA/CHELSA_transectos/pcp_transect_centr_df_1979_2019.csv",  
#            row.names = FALSE, fileEncoding = "ISO-8859-1")
...
resultados_fin <- resultados_fin[-1,]
write.csv2(resultados_fin, "C:/A_TRABAJO/ELIZA/CHELSA_transectos/pcp_comparation_regions.csv",row.names = FALSE, fileEncoding = "ISO-8859-1")
...
resultados_fin <- resultados_fin[-1,]
write.csv2(resultados_fin, "C:/A_TRABAJO/ELIZA/CHELSA_transectos/pcp_comparation_transects.csv", row.names = FALSE, fileEncoding = "ISO-8859-1")
